import sys
sys.path.append('/mnt/bn/yangmin-priv/luoruipu/code/Edit-GPT4/model/baselines')
from minigpt4.common.config import Config
from minigpt4.common.dist_utils import get_rank
from minigpt4.common.registry import registry
from minigpt4.conversation.conversation import Chat, CONV_VISION_minigptv2, StoppingCriteriaSub
...
class MiniGPT4_Interface(nn.Module):
    def __init__(self, model_path="facebook/opt-350m", model_type=None, device=None, half='fp16', inference_method='generation') -> None:
        ...
        self.conv = CONV_VISION_minigptv2.copy()

    def get_conv(self):
        return self.conv

    @torch.no_grad()
    def raw_generate(self, image_list, prompt, max_new_tokens=300, num_beams=1, min_length=1, top_p=0.9,
               repetition_penalty=1.0, length_penalty=1, temperature=1.0, max_length=2000):
        # conv.append_message(conv.roles[1], None)
        conv = self.conv.copy()
        first_query_fn = self.get_first_query_process()
        query = first_query_fn(prompt)
        ...
        img_list = [self.vis_processor(raw_image).unsqueeze(0).to(self.device).to(self.dtype) for raw_image in raw_images]
    ...
    def raw_predict(self, image_list, prompt, candidates, likelihood_reduction='sum'):
        ...
        try:
            ...
            candidates_emb = self.model.embed_tokens(candidates_ids)
