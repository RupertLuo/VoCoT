...
def extract_gt_action(example, cord='rel', use_xy:bool=False, use_click_box:bool=False):
    ex_action_type = example['result_action_type']

    if ex_action_type == ActionType.DUAL_POINT:
        lift_yx = example['result_lift_yx']
        touch_yx = example['result_touch_yx']
        if is_tap_action(np.array(touch_yx), np.array(lift_yx)):
            rel_x, rel_y = lift_yx[1], lift_yx[0]
            if   cord == 'rel': 
                if use_xy: 
                    action = f'click on ({rel_x:.2f},{rel_y:.2f})'
                    action_ground = f"action_point: ({rel_x:.2f},{rel_y:.2f})"
                else:
                    action = f'click on ({rel_y:.2f},{rel_x:.2f})'
                    action_ground = f"action_point: ({rel_y:.2f},{rel_x:.2f})"
            elif cord == 'abs': 
                if 'image' in example: height, width, __ = example['image'].shape
                elif 'image_size' in example: height, width = example['image_size']
                elif 'image_shape' in example: height, width, _ = example['image_shape']
                else: raise NotImplementedError('No image height and width information provided!')
                abs_x, abs_y = int(lift_yx[1] * width), int(lift_yx[0] * height)
                if use_xy: 
                    action = f'click on ({abs_x},{abs_y})'
                    action_ground = f"action_point: ({abs_x},{abs_y})"
                else:
                    action = f'click on ({abs_y},{abs_x})'
                    action_ground = f"action_point: ({abs_y},{abs_x})"
            if 'result_bbox' in example and use_click_box:
                # QWen-VL bounding box format: <box>(x1,y1),(x2,y2)</box>
                y1,x1,y2,x2 = example['result_bbox']
                action_ground += f", action_bbox: <box>({x1},{y1}),({x2},{y2})</box>"
            action_type = 'CLICK'
        else:
            action_type = 'SCROLL'
            ...
            if v_change >= 0.9*h_change:   # vertical
                if is_scroll_up: action, action_ground = "scroll up",   "direction: up"
                else:            action, action_ground = "scroll down", "direction: down"
            else:                          # horizonal
                if is_scroll_left: action, action_ground = "scroll left",  "direction: left"
                else:              action, action_ground = "scroll right", "direction: right"
        ...
        action_type = 'PRESS'
        action_ground = f'action_button: {button}'
    elif ex_action_type == ActionType.PRESS_ENTER:
        action = "press enter"
        action_type = 'PRESS'
        action_ground = f'action_button: enter'
    elif ex_action_type == ActionType.TYPE:
        action_text = example['result_action_text']
        action = f'type text "{action_text}"'
        action_type = 'TYPE'
        action_ground = f'action_text: "{action_text}"'
    elif ex_action_type == ActionType.STATUS_TASK_COMPLETE:
        action = 'stop and set the query as completed'
        action_type = 'STOP'
        action_ground = f'action_result: query completed'
    elif ex_action_type == ActionType.STATUS_TASK_IMPOSSIBLE:
        action = 'stop and set the query as failed'
        action_type = 'STOP'
        action_ground = f'action_result: query failed'
    else:
        raise NotImplementedError
    
    return action, action_type, action_ground
