...
def create_eva_vit_emu(model_path, load_ckpt=True):
    vision_config = json.load(open(os.path.join(model_path, 'config.json')))
    vision_config = Namespace(**vision_config)
    ...
    if load_ckpt:
        try:
            valid_path = Path(model_path)
            for p in tqdm.tqdm(valid_path.glob('pytorch_model_split*.bin'), desc='loading model splits'):
                current_ckpt = torch.load(p, map_location='cpu')
                visual.load_state_dict(current_ckpt, strict=False)
                del(current_ckpt)
            # ckpt = torch.load(os.path.join(model_path, 'pytorch_model.bin'), map_location='cuda')
            # interpolate_pos_embed(visual, ckpt)
            # unmatched_info = visual.load_state_dict(ckpt, strict=False)
            # print(unmatched_info)
        except:
            print('do not load the checkpoint')
    return visual