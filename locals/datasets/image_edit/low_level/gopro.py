...
class GoPro(Dataset):
    def __init__(self, path, split="train", size=256, interpolation="pil_lanczos", check=False, raw_image=False,
        ...
        filenames_splits = sorted(os.listdir(os.path.join(path, split)))
        
        self.inp_filenames = [os.path.join(path, split, d, 'blur', x) for d in filenames_splits for x in sorted(os.listdir(os.path.join(path, split, d, 'blur'))) if is_image_file(x)]
        self.tar_filenames = [os.path.join(path, split, d, 'sharp', x) for d in filenames_splits for x in sorted(os.listdir(os.path.join(path, split, d, 'sharp'))) if is_image_file(x)]
