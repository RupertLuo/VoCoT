...
class REDS(Dataset):
    def __init__(self, path, split="train", size=256, interpolation="pil_lanczos", check=False, raw_image=False,
        flip_prob=0.5, sample_weight=1.0, instruct=False, prompt_path='datasets/image_edit/prompt/prompt_deblur.txt'):
        super(REDS, self).__init__()

        inp_files = sorted(os.listdir(os.path.join(path, split, 'train_blur')))
        tar_files = sorted(os.listdir(os.path.join(path, split, 'train_sharp')))

        if split == "train":
            self.inp_filenames = [os.path.join(path, split, 'train_blur', d, x) for d in inp_files for x in sorted(os.listdir(os.path.join(path, split, 'train_blur', d))) if is_image_file(x)]
            self.tar_filenames = [os.path.join(path, split, 'train_sharp', d,  x) for d in tar_files for x in sorted(os.listdir(os.path.join(path, split, 'train_sharp', d))) if is_image_file(x)]
        else:
