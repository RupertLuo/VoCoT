...
class SIDD(Dataset):
    def __init__(self, path, split="train", size=256, interpolation="pil_lanczos", check=False, raw_image=False,
        flip_prob=0.5, sample_weight=1.0, instruct=False, prompt_path='datasets/image_edit/prompt/prompt_denoise.txt'):
        ...
        filenames_splits = sorted(os.listdir(os.path.join(path)))
        
        self.inp_filenames = [os.path.join(path, d, x) for d in filenames_splits for x in sorted(os.listdir(os.path.join(path, d))) if is_image_file(x) and 'NOISY' in x]
        self.tar_filenames = [os.path.join(path, d, x) for d in filenames_splits for x in sorted(os.listdir(os.path.join(path, d))) if is_image_file(x) and 'GT' in x]
    ...
    def get_raw_image(self, index):
        ...
        return {'input_images': [inp_img, tar_img], 'output_images': [tar_img], 'output_cond_images': [inp_img], 'conversation': sources, 'image_label_masks': [0, 1]}